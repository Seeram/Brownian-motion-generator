...
my $basic_gnuplot_file_points = <<"FILE_SETUP";
...
my $basic_gnuplot_file_trajectories = <<"FILE_SETUP";
set style line 1 lc rgb \'#0060ad\'
set terminal svg size 350,262 enhanced font \'Verdana,10\'
...
{
    my ($no_time_steps, $no_random_variables, $length) = @_;
...
{
    my ($no_trajectories, $no_time_steps, $no_random_variables, $length) = @_;

    my @trajectories;

    foreach (1..$no_trajectories) {
        push @trajectories, generate_brown_trajectory($no_time_steps, $no_random_variables, $length);
    }
...
{
    my ($no_trajectories, $no_time_steps, $no_random_variables, $length, $filename) = @_;	
    ...
    my $trajectories = generate_brown_trajectories($no_trajectories, $no_time_steps, $no_random_variables, $length);
...
sub run
{
    ...
    generate_graph_brown_trajectories($no_trajectories, $no_time_steps, $no_random_variables, $length, $trajectories_filename);
